...
{
  // V15 uses the configurable pwrOffSpeed delay only. The generic instant-off
  // shortcut (pwrForcePressed in PWR_PRESS_SHUTDOWN_DELAY) is for Horus-style
  // radios where either key starts shutdown and both keys force it off.
  return false;
}
#endif
...
{
#if defined(PWR_EXTRA_SWITCH_GPIO)
  // SYS+MDL must both be held; a single key must not trigger power actions.
